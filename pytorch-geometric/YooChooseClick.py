...
print('average length of session: '
      + str(df.groupby('session_id')['item_id'].size().mean()))

item_encoder = LabelEncoder()
df['item_id'] = item_encoder.fit_transform(df.item_id)
df['label'] = df.session_id.isin(buy_df.session_id)
print('average probability of buy after clicking during session: '
      + str(df.drop_duplicates('session_id')['label'].mean()))
...
class YooChooseBinaryDataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        return ['./yoochoose/yoochoose-clicks-processed/yoochoose_click_binary_1M_sess.dataset']
